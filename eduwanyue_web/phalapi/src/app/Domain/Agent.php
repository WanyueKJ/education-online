...
//
//        $codeinfo = $model->getAgent(['uid' => $uid]);
//
//        $where = ['pid' => $uid];
...
//        $where       = ['id' => $uid];
//        $userinfo    = $Domain_User->getUsersInfo($where, 'integral');
//        if ($userinfo) {
//            $income = $userinfo['integral'];
//        }
//        if ($income == 0) {
//            $income = '0';
//        }
//        $istips = '0';
//        if ($codeinfo['isagent'] == 0) {
//            $istips = '1';
//        }
//        if ($codeinfo['isagent'] == 1 && $codeinfo['istips'] == 1) {
//            $where2 = ['uid' => $uid];
//            $up     = ['istips' => 0];
//            $model->upCode($where2, $up);
//            $istips = '4';
//        }
//
...
//        $isshowauth   = '0';
//        if ($codeinfo['isagent'] == 0 && $agent_status == 3) {
//            $Domain_Auth = new Domain_Auth();
//            $where3      = ['uid' => $uid];
//            $field       = 'status';
//            $auth        = $Domain_Auth->getAuthInfo($where3, $field);
//            if (!$auth) {
//                $isshowauth = '1';
//            }
//            if ($auth) {
...
//
//        $tip_type = [
//            '1' => '平台内购买任意付费课程即可成为推广员',
//            '2' => '平台内累计消费金额达到¥' . $configpri['agent_money'] . '时即可成为推广员。',
//            '3' => '成为平台推广员需要上传个人信息进行认证。',
//        ];
//        $tips     = [];
//        $tips[]   = $tip_type[$agent_status] ?? '';
//        $tips[]   = '成为推广员后即可推广下级用户，下级用户在平台内产生消费行为后，推广员即可得到相应积分奖励。';
//
//        $info = [
//            'code'       => $code,
//            'income'     => $income,
//            'nums'       => $nums,
//            'isagent'    => $codeinfo['isagent'],
//            'istips'     => $istips,
...
//    }
    ...
    /* 列表 */
//    public function getAgentList($where, $p)
//    {
//        $model = new Model_Agent();
//        $field = 'uid';
//        $list  = $model->getAgentList($where, $p, $field);
//        foreach ($list as $k => $v) {
//            $list[$k] = \App\getUserInfo($v['uid']);
//        }
//
//        return $list;
//    }
