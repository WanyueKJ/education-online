...
//        }
        ...
        foreach ($categories as $item) {
            ...
            $action='<a class="btn btn-xs btn-primary" href="' . url("AdminCategory/edit", ["id" => $item['id']]) . '">' . lang('EDIT') . '</a> ';
            if($item['id']>1){
                $action.='<a class="btn btn-xs btn-danger js-ajax-delete" href="' . url("AdminCategory/delete", ["id" => $item['id']]) . '">' . lang('DELETE') . '</a> ';
            }
            $item['str_action']     = $action;
            if ($item['status']) {
